      ...
      <div className={`relative w-full ${image ? 'h-auto' : heightClasses[height]} overflow-hidden group flex`}>
        ...
        {image ? (
          <>
            <img
              src={image}
              alt={title || 'Lurevi Premium Digital Art Prints'}
        ...
        {/* If image banner with button but no title/subtitle, position button cleanly under OFF PRINTS */}
        {image && !title && !subtitle && buttonText && buttonLink ? (
          <div className="absolute left-[23%] top-[76%] sm:top-[78%] -translate-x-1/2 -translate-y-1/2 z-20">
            <Link
              to={buttonLink}
              className="inline-flex items-center justify-center px-6 sm:px-8 py-2.5 sm:py-3.5 rounded-full text-xs sm:text-sm font-semibold bg-white text-gray-900 hover:bg-gray-100 transition-all duration-300 transform hover:scale-[1.03] shadow-md hover:shadow-lg font-sans border border-gray-100"
            >
          ...
          <div className="absolute inset-0 z-20 w-full h-full flex items-center">
            <div className={`max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 flex flex-col ${alignmentGridClasses[alignment || 'left']}`}>
              {title && (
                <h1
              ...
              {buttonText && buttonLink && (
                <div className="flex shrink-0">
                  <Link
                    to={buttonLink}
                    className={`inline-flex items-center justify-center px-5 sm:px-7 py-2.5 sm:py-3.5 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 transform hover:scale-[1.03] shadow-md hover:shadow-lg ${
                      isLight
