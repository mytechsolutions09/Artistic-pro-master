...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://lurevi.in/collections/${categorySlug}`,
      languages: {
        'en-IN': `https://lurevi.in/collections/${categorySlug}`,
        'x-default': `https://lurevi.in/collections/${categorySlug}`,
      },
    },
    openGraph: {
      title: titleText,
...
export default async function CollectionPage({ params }: Props) {
  ...
  try {
    ...
    console.error('Error fetching category/products for CollectionPage:', err);
  }

  // If the collection doesn't exist and it's not the static luxury page, return 404
  if (!category && categorySlug !== 'luxury-wall-art') {
    return notFound();
  }
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              '@graph': [
                {
                  '@type': 'CollectionPage',
                  '@id': `https://lurevi.in/collections/${categorySlug}/#webpage`,
                  url: `https://lurevi.in/collections/${categorySlug}`,
                ...
                },
                {
                  ...
                  itemListElement: [
                    ...
                    { '@type': 'ListItem', position: 2, name: 'Collections', item: 'https://lurevi.in/collections' },
                    { '@type': 'ListItem', position: 3, name: finalCategory.name, item: `https://lurevi.in/collections/${categorySlug}` },
                  ],
                },
              ],
            }),
